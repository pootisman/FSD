...
#define HELP_MESSAGE                                                           \
  ...
  "sampling rate per second\n--dynamic Enable dynamic coloring of the cube\n"

GLfloat *color_cube = NULL;

/* Allowed options */
const struct option ALLOWED[14] = {
    ...
    {"coloring", 1, NULL, 12},  {"dynamic", 0, NULL, 13}};
...
int main(int argc, char *argv[]) {
  ...
  vinyl_prep(window_size * 3, points_per_step, run_loop, color_cube,
             CUBE_POINTS, cube_ready, red_set, green_set, blue_set, figure_mode, sample_rate, timeoutval);
             
  if(pattern_name != NULL){
    load_pattern(pattern_name);
  }

  if(dynamic_coloring != 0){
    vinyl_dynamic_coloring(dynamic_coloring);
  }
